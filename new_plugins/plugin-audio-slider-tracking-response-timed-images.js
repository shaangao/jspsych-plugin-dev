// adapted from jsPsych 8.2.1 AudioSliderResponse by Shan Gao


var jsPsychAudioSliderTrackingResponse = (function (jspsych) {
	'use strict';
	...
	const info = {
	  ...
	  parameters: {
		...
		/** If true, then the slider history will be recorded. */
		track_slider_history: {
			type: jspsych.ParameterType.BOOL,
			default: false
		},
		/** For concurrently displaying images at image_timing for image_durations. */
		image_series: { 
			type: jspsych.ParameterType.IMAGE, 
			default: [], 
			array: true 
		},
		image_timing: {   // latency of image onset
			type: jspsych.ParameterType.INT, 
			default: [],  // in ms
			array: true 
		},
	...
	class AudioSliderTrackingResponsePlugin {

	  constructor(jsPsych) {

	    this.jsPsych = jsPsych;
	    // this.response = { rt: null, response: null };  // i have no idea why this is repeated later
	    ...
	    this.setupTrial = () => {
		  ...
		  //////////// set up display elements & logics ////////////

	      var html = '<div id="jspsych-audio-slider-response-wrapper" style="margin: 100px 0px;">';
		  // image container 
		  html += '<div id="image-container" style="height: 200px; text-align: center; margin-bottom: 20px; display: flex; align-items: center; justify-content: center; position: relative;"></div>';
		  // slider
	      ...
	      html += '">';
	      html += 
		  	'<input type="range" class="jspsych-slider" value="' + this.params.slider_start + 
			'" min="' + this.params.min + 
			'" max="' + this.params.max + 
			'" step="' + this.params.step + 
			'" id="jspsych-audio-slider-response-response"';  // actual slider bar here
		  ...
		  // button
		  if (this.params.trial_ends_after_audio) {
			html += '<br> <button id="jspsych-audio-slider-response-next" style="display:none" class="jspsych-btn"></button>';
		  } else {
			...
			html += '<br> <button id="jspsych-audio-slider-response-next" class="jspsych-btn" ' + next_disabled_attribute + ">" + this.params.button_label + "</button>";
		  }
	   	  this.display.innerHTML = html;

		  // should the slider and button be enabled? 
	      if (!this.params.response_allowed_while_playing) {
	        this.display.querySelector("#jspsych-audio-slider-response-response").disabled = true;  // slider
	        this.display.querySelector("#jspsych-audio-slider-response-next").disabled = true;  // button
	      }
	    ...
	    //   };
		  ...
		  // what does the button click event trigger? 
		  ...
		  // start audio and record startTime
	      this.startTime = performance.now();
	      if (this.context !== null) {
	        this.startTime = this.context.currentTime;
	      }
	      this.audio.play();
		  console.log('playing audio at ' + this.startTime + ' ms');

		  // tracking slider history?
		  if (this.params.track_slider_history) {
			console.log("tracking slider history");
			// don't want users to be able to click or drag slider
			$(document).on('click', '#jspsych-audio-slider-response-response', function (event) {
				event.preventDefault();
			});
			$(document).on('mousedown', '#jspsych-audio-slider-response-response', function (event) {
				event.preventDefault();
			});
			$(document).on('mousedown', function (event) {
				event.preventDefault();
			});
			// variable for storing dynamic slider responses
			this.response = {
				rt: [0],
				response: [this.params.slider_start],
			}; // record start position
			// track changes
			const record_slider_position = () => {
				let this_rt = Math.round(performance.now() - this.startTime);
				let this_response = this.display.querySelector("#jspsych-audio-slider-response-response").valueAsNumber;
				this.response.rt.push(this_rt);
				this.response.response.push(this_response);
				console.log("slider position changed to " + this_response + " at " + this_rt + " ms after audio startTime.");
			};
			this.display.querySelector("#jspsych-audio-slider-response-response").addEventListener(
				"change", 
				record_slider_position
			);
		  } else {
			// variable for storing slider response
			this.response = {
				rt: null,
				response: null
			  };
		  ...
		  // show timed images after a set amount latency
		  this.params.image_timing.forEach((latency, index) => {
		  ...
		  const showImage = (index) => {
			if (index < this.params.image_series.length) {
			  var img = '<img src="' + this.params.image_series[index] + '" style="max-width: 100%; max-height: 100%;">';
			  document.getElementById('image-container').innerHTML = img;
			  // remove image after image_duration
			  jsPsych.pluginAPI.setTimeout(
				() => { document.getElementById('image-container').innerHTML = ''; }, 
				this.params.image_durations[index]
			  );
			}
		  };
		  
		  // end trial after trial_duration
		...
		// what do you do at the end of trial?
	    this.end_trial = () => {
	...
	return AudioSliderTrackingResponsePlugin;
